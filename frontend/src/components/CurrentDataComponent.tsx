	...
	return (
		<div className="bg-[#e5e5e5] m-5 rounded-[10px] w-[85%] h-42">
			...
			<div className="flex flex-row">
				...
				<p className="ml-2 font-[Inter] font-extrabold text-[#6d7278] text-[23px]" >Time: <span className="text-black">{currentData?.time ? (new Date(currentData.time).getHours().toString() + ":" + new Date(currentData.time).getMinutes().toString()) : 'not_found'}</span></p>
			</div>
		</div>
	);
};
